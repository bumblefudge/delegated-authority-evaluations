...
[Delegated SD-JWT](https://datatracker.ietf.org/doc/draft-gco-oauth-delegate-sd-jwt/) ("dSD-JWT", an individual IETF draft at revision 00) is an extension to Selective Disclosure JWT (SD-JWT) that lets the holder of an issued credential hand a bounded, verifiable authorization to another party without involving the issuer. Rather than defining a new credential type, it appends a delegation hop to an existing SD-JWT as an additional disclosure. Each hop is a JSON payload chosen by the delegator, wrapped in a Key Binding JWT signed by the key the previous hop named in its `cnf` claim, and hash-bound to the prior state of the credential (via `sd_hash` or `issuer_jwt_hash`). A hop may name a further `cnf` key, in which case the chain can continue, or omit it, in which case the chain terminates at that delegatee.
